...
- **Reminders:** Enable daily reminders and choose one or more local times in Settings. A Firefox browser notification appears at each configured time, including when no words are due; clicking it opens Practice in a full tab. A `!` badge on the abhyas toolbar icon remains until you open the extension, providing an in-browser reminder indicator. Settings shows the next scheduled reminder and a **Test browser notification** button. Firefox must be running and OS notifications must be allowed. A missed reminder is sent once when the extension next starts, then daily scheduling resumes. Disabling reminders cancels the alarm; the test button works independently of that setting.
...
Enable daily reminders, add one or more local times, and click **Save preferences**. Check the displayed **Next browser reminder** date/time: a time that has already passed is scheduled for tomorrow. Firefox browser notifications are delivered through the operating system, so the extension cannot force a visible banner if OS notifications or Focus/Do Not Disturb hide it. The `!` toolbar badge remains available as an in-browser indicator. Notification API errors are displayed in Settings; failed delivery still schedules the next time. Reminders work while the popup is closed. Temporary add-ons must be loaded again after restarting Firefox before any reminder can run.
...
1. Create a Supabase project. Do not enable or configure Supabase Auth; this extension does not use it.
2. In that project, open **SQL Editor**, paste the complete current [`supabase/setup.sql`](supabase/setup.sql), and select **Run**. It creates the `abhyas_token_sync` table and the `public.abhyas_token_exchange` database RPC; it also enables RLS, revokes direct table access, grants only RPC execution to the anonymous/publishable role, and refreshes PostgREST’s schema cache. There is no Edge Function to create.
3. Confirm that the query completed successfully. If the extension reports a missing sync function (404), run the entire SQL file again in the SQL Editor for the exact Project URL you entered.
4. Copy that project’s **Project URL** and **publishable key** from its API settings. A legacy `anon` key also works. Never use a `service_role` or `sb_secret_…` key in the extension.
5. Reload the extension. In **Settings → Sync with Supabase**, enter those values. Choose **Generate token** for the first device, then copy the token into a password manager. You may instead enter an existing valid token.
6. Choose **Connect and sync** and grant access to that project plus consent to the optional data transfer. From the popup, **Configure sync in full view** opens the setup form in a tab.
7. On another device, enter the same project URL, publishable key, and sync token. It will address the same remote snapshot. If that device already has local words, export a JSON backup before its first connection because the newer whole snapshot wins.
...
If sync reports that the function was not found (404), verify that the saved Project URL belongs to the project where you ran the current `supabase/setup.sql`, then run that entire file again in its SQL Editor. Sync requests originate in the extension background page, so they appear in Firefox's extension debugging tools (`about:debugging` → This Firefox → abhyas → Inspect), not in the popup or full-page Network inspector.
