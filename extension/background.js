(() => {
  ...
  function addWord(state2, data) {
    const word = data.word?.trim(), translation = data.translation?.trim();
    if (!word || !translation)
      throw new Error("Enter both a word and its English translation.");
    if (!state2.lists.some((l) => l.id === data.listId))
      throw new Error("Choose an existing list.");
    if (data.id) {
      const existing = state2.words.find((w) => w.id === data.id);
  ...
  function reviewWord(state2, id, rating, expectedReps, now = /* @__PURE__ */ new Date()) {
  ...
  function nextReminder(time, now = /* @__PURE__ */ new Date()) {
    const [h, m] = time.split(":").map(Number), next = new Date(now);
  ...
  function importBackup(state2, input) {
    const source = validateBackup(input), lists = [], words = [], reviews = [], listMap = /* @__PURE__ */ new Map(), wordMap = /* @__PURE__ */ new Map();
    ...
    state2.lists.push(...lists);
    state2.words.push(...words);
    state2.reviews.push(...reviews);
    return { added: words.length, listId: lists[0].id };
  }
  function importRows(state2, { name, rows }) {
    if (typeof name !== "string" || !name.trim() || !Array.isArray(rows) || !rows.length || rows.length > MAX_WORDS)
      throw new Error("Provide a list name and between 1 and 50,000 words.");
    const id = crypto.randomUUID(), addition = { lists: [{ id, name: name.trim() }], words: [] };
    ...
    state2.lists.push(...addition.lists);
    state2.words.push(...addition.words);
  ...
  function nextRevision(previous = EMPTY_REVISION, now = Date.now()) {
    return {
  ...
  function compareRevision(a, b) {
    return a.revision - b.revision || (a.change_id > b.change_id ? 1 : a.change_id < b.change_id ? -1 : 0);
  }
  ...
  async function syncIdentity(config, token) {
...
${validateSyncToken(token)}`
  ...
  function validateRemote(row) {
    if (!row || !Number.isSafeInteger(row.revision) || row.revision < 0 || !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/.test(
      row.change_id
    ))
  ...
  async function exchange(config, syncToken, version2, data) {
    const payload = validateBackup(snapshot(data));
    if (new TextEncoder().encode(JSON.stringify(payload)).length > 20 * 1024 * 1024)
      throw new Error(
    ...
    const result = await request(config, "/rest/v1/rpc/abhyas_token_exchange", {
      ...
      p_revision: version2.revision,
      p_change_id: version2.change_id,
      p_payload: payload
    });
  ...
  // src/sync-service.js
  function createSyncService(api) {
    const read = async () => api.storage.local.get(["data", "syncMeta", "syncAuth", "syncStatus"]);
      ...
      if (saved.syncAuth && !saved.syncAuth.token) {
        ...
        if (saved.syncMeta) {
          ...
          await api.storage.local.set({
            ...
            syncStatus: {
              connected: false,
              message: "Sync authentication changed. Connect once with a shared sync token."
            }
          });
        }
        saved.syncAuth = void 0;
      }
      if (!saved.syncMeta)
        await api.storage.local.set({
          syncMeta: data.words.length || data.reviews.length ? nextRevision() : { ...EMPTY_REVISION }
        });
      if (saved.syncAuth)
    ...
    async function run() {
      ...
      try {
        if (!await api.permissions.contains({
          origins: [config.url + "/*"],
        ...
        const local = saved.syncMeta || EMPTY_REVISION, comparison = compareRevision(remote, local);
        if (comparison < 0)
          throw new Error(
        ...
        const syncStatus = await status(
          comparison > 0 ? "Downloaded newer remote data. Previous local data is available below." : "Remote is up to date with this browser.",
          {
            ...info2,
        ...
        const syncStatus = await status(e.message, { ...info2, error: true });
        return { data: (await read()).data, syncStatus };
      }
    }
    async function connect(message) {
      ...
      const saved = await read(), owner = await syncIdentity(config, token);
      if (saved.syncMeta?.owner && /^[0-9a-f]{64}$/.test(saved.syncMeta.owner) && saved.syncMeta.owner !== owner)
        throw new Error(
  ...
  var sync = createSyncService(browser);
  var dataActions = /* @__PURE__ */ new Set([
    "importBackup",
    "importRows",
    "saveWord",
    "deleteWord",
  ...
  async function handle(message) {
    ...
    switch (message.type) {
      ...
      case "testNotification":
        ...
        break;
      case "practiceSize":
        if (!Number.isInteger(message.size) || message.size < 1 || message.size > 100)
  ...
  browser.contextMenus.onClicked.addListener((info) => {
    if (info.menuItemId === "add-word")
      browser.windows.create({
        url: browser.runtime.getURL("index.html") + "?capture=" + encodeURIComponent(info.selectionText || ""),
        type: "popup",
        width: 520,
        height: 740
      });
